...
# BUSCAR MALLAS CURRICULARES POR PERIODO LECTIVO
  ...
  def historial
    ...
    ultimos_periodos = Periodo.last(2)
    ultimo_periodo = ultimos_periodos.first
    @bandera = ""
    @matricula_periodo_anterior = Matricula.where(:persona_id => params[:estudiante]).last(2)
    @matricula_periodo_anterior = @matricula_periodo_anterior.first
    if (@matricula_periodo_anterior.curso.periodo.id == ultimo_periodo.id) and (ultimos_periodos.count()>1)
      @bandera = "pertenece"
    else
      @bandera = "no_pertenece"
    end
  end
